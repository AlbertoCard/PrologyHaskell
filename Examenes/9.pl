...
%cualquiera de los jugadores come fichas
%come 2 veces seguidas

%predicados y hechos nuevos

%variable para contar las veces que se come una ficha
%se usa en el predicado puedeColocarFicha y robarFicha
:-dynamic contador/1.
contador(0).

%la validacion se encuantra en cambiarTurno
%si el contador es mayor a 4 se termina el juego
comidas:-
    contador(X),
    X > 2,
    write("Se  tomaron mas de 2 fichas"), nl.
...
asignarFichas:-
    retract(jugador(jugador1,_)), assertz(jugador(jugador1, [[5,5],[6,6]])),
    retract(jugador(jugador2,_)), assertz(jugador(jugador2, [[4,4],[3,3]])),
...
repartir:- 
    ...
    fichas(Fichas),
    repartirFichas(Fichas, 2, FichasJ1, FichasJ2),
    imprimirFichasJugador("jugador1", FichasJ1), 
    imprimirFichasJugador("jugador2", FichasJ2), 
...
cambiarTurno(jugador1):-
    ...
    comidas -> terminar_juego
    ;
        (retract(turno(_)), assertz(turno(jugador2)),
        retract(contador(_)), assertz(contador(0)),
...
robarFicha(Jugador):-
    ...
    X1 is Contador + 1,
    retract(contador(_)), assertz(contador(X1)),
    write("Contador: "), write(X1), nl,
