...
%Uno de los jugadores pone una ficha 
%que genera puntaje dos veces

%predicados y hechos nuevos
%variable para contar si genera puntaje
%se actualiza en el predicado sumarPuntaje
:- dynamic puntajeGenerado/2.
puntajeGenerado(jugador1, 0).
puntajeGenerado(jugador2, 0).

%validar si sumo puntaje 2 veces
%se usa en el predicado cambiarTurno
sumaPuntaje(Jugador):-
    puntajeGenerado(Jugador, X),
    X = 2,
    write("El jugador "), write(Jugador), write(" ha generado puntaje 2 veces"), nl.
...
asignarFichas:-
    retract(jugador(jugador1,_)), assertz(jugador(jugador1, [[0,0],[1,6],[0,2],[0,3],[0,4]])),
    retract(jugador(jugador2,_)), assertz(jugador(jugador2, [[5,5],[1,2],[1,3],[0,1],[1,5]])),
...
cambiarTurno(jugador1):-
    ...
    sumaPuntaje(jugador1) -> terminar_juego
    ;
...
cambiarTurno(jugador2):-
    ...
    write("Puntaje del jugador 2: "), write(P2), nl,
    retract(turno(_)),
    sumaPuntaje(jugador2) -> terminar_juego
    ;
...
sumarPuntaje(jugador1):-
    ...
    PuntajeJ1N > PuntajeJ1 -> 
        (X1 is X + 1, retract(puntajeGenerado(jugador1, _)), assertz(puntajeGenerado(jugador1, X1))).

sumarPuntaje(jugador2):-
    puntajeGenerado(jugador2, X),
    puntaje([PuntajeJ1, PuntajeJ2]),
    extremos([Arr, Ab, Iz, De]),
    mula([MulaArr, MulaAb, MulaIz, MulaDe]),
    Modulo is (Arr * MulaArr) + (Ab * MulaAb) + (Iz * MulaIz) + (De * MulaDe),
    ...
    PuntajeJ2N > PuntajeJ2 -> 
        (X1 is X + 1, retract(puntajeGenerado(jugador2, _)), assertz(puntajeGenerado(jugador2, X1))).
