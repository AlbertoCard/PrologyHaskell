%Para probar este programa se debe correr asignarFichas
%Este programa termina cuando:
%Se ponen fichas en un mismo extremo
%3 veces seguidas

%predicados y hechos nuevos
%variable para contar las veces que se han colocado en cada extremo
%se actualiza en el predicado puedeColocarFicha
:- dynamic contadorExtremo/1.
%contadorExtremo([Arr, Abj, Izq, Der])
contadorExtremo([0,0,0,0]).

%validar si van 3 seguidas
%se usa en el predicado cambiarTurno
extremoSeguidas:-
    contadorExtremo([Arr, Abj, Izq, Der]),
    Total is Arr + Abj + Izq + Der,
    Total >= 3,
    write("Se han colocado 3 fichas en un mismo extremo"), nl.
...
asignarFichas:-
    retract(jugador(jugador1,_)), assertz(jugador(jugador1, [[5,5],[0,1],[1,6],[0,3],[0,4]])),
    retract(jugador(jugador2,_)), assertz(jugador(jugador2, [[0,6],[1,2],[1,3],[1,4],[1,5]])),
...
cambiarTurno(jugador1):-
    ...
    extremoSeguidas -> terminar_juego
    ;
...
puedeColocarFicha(Jugador) :-
    extremos([Arr, Ab, Iz, De]),
    jugador(Jugador, Fichas),
    contadorExtremo([ArrC, AbC, IzC, DeC]),
    (
            member([Arr, _], Fichas) -> (
                                            Lugar = "arriba", ArrC1 is ArrC + 1,
                                            retract(contadorExtremo(_)), assertz(contadorExtremo([ArrC1, 0, 0, 0]))
                                        )
        ;   member([Ab, _], Fichas) ->  (
                                            Lugar = "abajo", AbC1 is AbC + 1,
                                            retract(contadorExtremo(_)), assertz(contadorExtremo([0, AbC1, 0, 0]))
                                        )
        ;   member([Iz, _], Fichas) ->  (
                                            Lugar = "izquierda", IzC1 is IzC + 1,
                                            retract(contadorExtremo(_)), assertz(contadorExtremo([0, 0, IzC1, 0]))
                                        )
        ;   member([De, _], Fichas) ->  (
                                            Lugar = "derecha", DeC1 is DeC + 1,
                                            retract(contadorExtremo(_)), assertz(contadorExtremo([0, 0, 0, DeC1]))
                                        )
        ;   member([_, Arr], Fichas) -> (
                                            Lugar = "arriba", ArrC1 is ArrC + 1,
                                            retract(contadorExtremo(_)), assertz(contadorExtremo([ArrC1, 0, 0, 0]))
