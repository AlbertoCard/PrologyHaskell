...
%Hay fichas en los extremos que formentan una escalera
%por ejemplo: 1,2,3,4 o 4,3,2,1

%predicados y hechos nuevos

%orenarlos de menor a mayor
%se usa en el predicado cambiarTurno
ordenarExtremos:-
    extremos([Arr, Ab, Iz, De]),
    sort([Arr, Ab, Iz, De], Nueva ),
    (Nueva = [0, 1, 2, 3];
    Nueva = [1, 2, 3, 4];
    Nueva = [2, 3, 4, 5];
    Nueva = [3, 4, 5, 6]) ->
    write("Se ha formado la escalera"), write(Nueva), nl.
...
asignarFichas:-
    retract(jugador(jugador1,_)), assertz(jugador(jugador1, [[5,5],[3,5],[1,5],[0,0],[0,6]])),
    retract(jugador(jugador2,_)), assertz(jugador(jugador2, [[4,5],[2,5],[2,6],[1,6],[1,2]])),
...
cambiarTurno(jugador1):-
    ...
    ordenarExtremos -> terminar_juego
    ;
