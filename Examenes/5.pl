...
%Se colocan dos mulas seguidas

%predicados y hechos nuevos
%variable para contar las mulas consecutivas
%se actualiza en el predicado colocarFicha
:- dynamic mulasConsecutivas/1.
mulasConsecutivas(0).

%validar si van 2 seguidas
%se usa en el predicado cambiarTurno
mulasSeguidas:-
    mulasConsecutivas(N),
    N = 2,
    write("Se han colocado 2 mulas seguidas"), nl.
...
asignarFichas:-
    retract(jugador(jugador1,_)), assertz(jugador(jugador1, [[5,5],[2,5],[2,2],[0,3],[0,4]])),
...
colocarFicha(Jugador, Lugar) :-
    ...
    Lugar == "arriba" -> 
        (retract(extremos([_, Ab, Iz, De])), assertz(extremos([Extremo, Ab, Iz, De])),
            (
                Extremo = Arr -> retract(mula(_)), assertz(mula([2, MulaAb, MulaIz, MulaDe])), 
                N1 is N+1, retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(N1))
                ; 
                retract(mula(_)), assertz(mula([1, MulaAb, MulaIz, MulaDe])),
                retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(0))
            ),
    ...
    ;   Lugar == "abajo" -> 
        (retract(extremos([Arr, _, Iz, De])), assertz(extremos([Arr, Extremo, Iz, De])),
            (
                Extremo = Ab -> retract(mula(_)), assertz(mula([MulaArr, 2, MulaIz, MulaDe])),
                N1 is N+1, retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(N1))
                ; 
                retract(mula(_)), assertz(mula([MulaArr, 1, MulaIz, MulaDe])),
                retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(0))
            ),
    ...
    ;   Lugar == "izquierda" -> 
        (retract(extremos([Arr, Ab, _, De])), assertz(extremos([Arr, Ab, Extremo, De])),
            (
                Extremo = Iz -> retract(mula(_)), assertz(mula([MulaArr, MulaAb, 2, MulaDe])),
                N1 is N+1, retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(N1))
                ; 
                retract(mula(_)), assertz(mula([MulaArr, MulaAb, 1, MulaDe])),
                retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(0))
            ),
    ...
    ;   Lugar == "derecha" -> 
        (retract(extremos([Arr, Ab, Iz, _])), assertz(extremos([Arr, Ab, Iz, Extremo])),
            (
                Extremo = De -> retract(mula(_)), assertz(mula([MulaArr, MulaAb, MulaIz, 2])),
                N1 is N+1, retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(N1))
                ; 
                retract(mula(_)), assertz(mula([MulaArr, MulaAb, MulaIz, 1])),
                retract(mulasConsecutivas(_)), assertz(mulasConsecutivas(0))
            ),
